...
const ablyManager = require('./ablyManager'); 

router.post('/create-table', async (req, res) => {
    try {
        await createNumbersTable();
        ablyManager.publish('table-created', JSON.stringify({ message: 'Tabla creada con éxito.' }));
...
router.post('/insert-number', async (req, res) => {
    try {
        ...
        ablyManager.publish('number-created', JSON.stringify({ message: 'Número insertado con éxito.' }));
